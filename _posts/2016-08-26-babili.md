...
We released [Babili](https://github.com/babel/minify) as beta (0.0.1) a few days ago under an MIT license!
...
> If you can't remember the name, babel-minify works too (made [an issue](https://github.com/babel/minify/issues/124) for the name).
...
It is written as a set of Babel plugins, consumable with the [`babel-minify` preset](https://github.com/babel/minify/tree/master/packages/babel-preset-minify).
...
### [Some of the Plugins](https://github.com/babel/minify#individual-plugins)
...
If you already use Babel, you can just add the [`babel-minify`](https://github.com/babel/minify#babel-preset) preset (`babel-preset-minify`) to your config.
...
would lose the speed benefits (needs to be measured) of doing the transpiling/minification in the same step. Again, this is something we need to think about: how to intergrate or pass more information to the bundler.

### Babili CLI

If you don't use Babel, you can use our standalone CLI tool [`babel-minify`](https://github.com/babel/minify#cli). (Currently it's just a wrapper for `babel-cli` + the preset). You could run this after transpiling (or not) in place of Uglify.
...
$ minify src -d lib
# equivalent to
# babel src -d lib --presets=minify --no-babelrc
```
...
```bash
$ npm install babel-core babel-loader babel-preset-minify
```
...
Or use it seperately with the [babel-minify-webpack-plugin](https://github.com/webpack-contrib/babel-minify-webpack-plugin) (made by [@boopathi](https://github.com/boopathi/), who also works on Babili).
...
module.exports = {
  ...
  output: //...,
  plugins: [
    new MinifyPlugin(options)
  ]
}
```

We want to have a better story with integration with Webpack/bundlers in the near future! Also check out [#100](https://github.com/babel/minify/issues/100).
...
- Each minification step can be split into its own plugin, and there’s plenty of options for customization. This makes it easier to contribute and to find/submit issues for specific problems. It also means that people can independently create their own experimental plugins before upstreaming them into core.
  - For example: [this](https://github.com/babel/minify/tree/master/packages/babel-plugin-transform-minify-booleans) just turns `true` into `!0` which is straightforward to write.
