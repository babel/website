...
I'l be writing up the 7.0 post (current progress and what's next) soon so look out for that! We also are starting our [7.0 upgrade guide](https://github.com/babel/babel.github.io/pull/1146).

We want to do whatever we can to make it easy for both end-users and plugin/tool authors to upgrade seamlessly: whether it be a beta period for the release, an upgrade guide, codemods if necessary, a bot to automatically upgrade your OSS project from 6 to 7, or any other suggestions you can give us!

> Our [team](https://github.com/babel/babel#team) is still a small volunteer group of people. There's no company sponsor or even anyone working on it full time. If you can spend time (especially at work) to contribute, reach out to us! We have a [Slack](https://slack.babeljs.io) and [Twitter](https://twitter.com/babeljs).
...
`<this.component />` was previously hoisted outside it's own function (doesn't make sense because otherwise `this` would be `undefined`)
