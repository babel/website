...
| `-i, --ignore [path]`  |                                   | Ignore all files that are in this path when using the require hook. If both `-i` and `-o` are omitted, files outside the current working directory are also ignored.                           |
| `-o, --only [path]`    | current working directory         | Only include (and exclude all other) files that are in this path when using the require hook. If both `-i` and `-o` are omitted, files outside the current working directory are also ignored. |
