...
```
Most popular presets are: [`env`](./preset-env.mdx), [`react`](./preset-react.mdx), [`typescript`](./preset-typescript.mdx), [`flow`](./preset-flow.mdx). You can also use `Babel.availablePresets` to query available presets.
...
Use the `data-targets` attribute to specify the [compilation targets](./options.mdx#targets) of the project.
...
Load `babel.js` or `babel.min.js` in your environment. This will expose [Babel's API](./core.mdx) in a `Babel` object:
...
Note that [config files](config-files.mdx) don't work in `@babel/standalone`, as no file system access is available. The presets and/or plugins to use **must** be specified in the options passed to `Babel.transform`.
