...
> Also check out the [v7-migration guide](v7-migration.mdx) for other user-level changes.
...
Check the `babel.config.js` docs for more info: [project-wide configuration](config-files.mdx#project-wide-configuration)

This file combined with the new [`overrides`](options.mdx#overrides) property and `env` lets you have a single config file
...
See Babylon's [plugin options](parser.mdx#plugins).
