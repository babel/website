...
:::tip
If you are relying on browserslist's defaults query (either explicitly or by having no browserslist config), you will want to check out the [No targets](options.mdx#no-targets) section for information on preset-env's behavior.
:::
