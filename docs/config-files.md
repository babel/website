...
for a `"babel.config.js"` or `"babel.config.json"` in this root directory. Alternatively, users can use an explicit
...
### `babel.config.js` vs `babel.config.json`
...
files or just a central [`babel.config.js`](#project-wide-configuration) or 
...
they are not needed in Babel 7, in favor of a [`babel.config.js`](#project-wide-configuration) or 
[`babel.config.json`](#project-wide-configuration).

### Root `babel.config.js` and `babel.config.json` files

The first step in any monorepo structure should be to create a
[`babel.config.js`](#project-wide-configuration) or [`babel.config.json`](#project-wide-configuration) 
...
The first issue you'll likely run into is that by default, Babel expects to load [`babel.config.js`](#project-wide-configuration) and [`babel.config.json`](#project-wide-configuration)
files from the directory set as its ["root"](options.md#root), which means that if you create
a [`babel.config.js`](#project-wide-configuration) or [`babel.config.json`](#project-wide-configuration), but run 
...
the working directory upward looking for your [`babel.config.js`](#project-wide-configuration) or
[`babel.config.json`](#project-wide-configuration) file,
...
Similar to the the way `babel.config.js` and `babel.config.json` files are required to be in the ["root"](options.md#root),
`.babelrc` files must be in the root _package_, by default. This means that, the same way the
working directory affects `babel.config.js` and `babel.config.json` loading, it also affects `.babelrc` loading.

Assuming you've already gotten your `babel.config.js` or `babel.config.json` file loaded properly as discussed above,
...
option from inside your `babel.config.js` or `babel.config.json` file to do
