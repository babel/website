...
**NOTE:** [Visitor aliases](#visitor-aliases) are also honored so you can use `t.isFunction`.
...
export default new Transformer("foo-bar", {
  FunctionDeclaration(node, parent) {
    this.parentPath.replaceWith(t.expressionStatement(t.literal("no function declarations allowed!"));
  }
});
```
