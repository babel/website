...
| `env`                    | `{}`                 | <p>This is an object of keys that represent different environments. For example, you may have:</p> `{ env: { production: { optional: ["optimisation", "minification"] } } }`<p>which will use those options when the enviroment variable <code>BABEL_ENV</code> is set to <code>"production"</code>. If <code>BABEL_ENV</code> isn't set then <code>NODE_ENV</code> will be used, if it's not set then it defaults to <code>"development"</code></p> |
