---
layout: post
title:  "7.9.0 Released: `@babel/preset-modules`, TypeScript 3.8 support and a new JSX transform"
author: Nicolò Ribaudo
authorURL: https://twitter.com/NicoloRibaudo
date:   2020-03-13 0:00:00
categories: announcements
share_text: "Babel 7.9.0 Released"
---

While preparing the Babel 8.0.0 [release](https://github.com/babel/babel/issues/10746) (in the next months), we just finished working on a new minor release which includes updates related to preset-modules, TS 3.8, Flow, and JSX!
...
*Note: These optimizations will be enabled by default in Babel 8. They can be manually enabled in Babel 7.9 by passing `{bugfixes: true}` to `preset-env`.*
...
We also worked with the React team to provide a new [JSX transform](https://github.com/reactjs/rfcs/blob/createlement-rfc/text/0000-create-element-changes.md), which will make it possible for React and React-like libraries to further optimize the creation of JSX elements.

Lastly, `@babel/parser` now supports two additional ECMAScript proposals: [Record & Tuple](https://github.com/tc39/proposal-record-tuple) and [Module Attributes](https://github.com/tc39/proposal-module-attributes). Please note taht this is only parser support, and the transforms are still being worked on.
...
Special thanks go to [Luna Ruan](https://twitter.com/lunaruan) from the React Team ([Facebook](https://opensource.facebook.com/)) for contributing the new JSX transform, to [Rick Button](https://twitter.com/rickbutton) ([Bloomberg](https://www.techatbloomberg.com/)) for implementing parser support for the [Record & Tuple](https://github.com/tc39/proposal-record-tuple) proposal, and to [Vivek Nayyar](https://twitter.com/VivekNayyar09) who worked on [Module Attributes](https://github.com/tc39/proposal-module-attributes)!
...
Starting from version [TODO], React supports a group of new functions for instantiating JSX elements, as an alternative to the legacy general-purpose `React.createElement` function. This will allow optimizing them better in the future
...
Additionally, the new transform automatically imports `"react"` (or other libraries which support the new API) when needed, so you don't have to manually include it anymore.
...
> **Note:** The functions inside `react/jsx-runtime` and `react/jsx-dev-runtime` are not meant to be used outside the `@babel/plugin-transform-react-jsx` and `@babel/plugin-transform-react-jsx-development` plugins.


You can enable this new transform by passing `{ "runtime": "automatic" }` (as opposed to `"classic"`) to `@babel/plugin-transform-react-jsx` or `@babel/preset-react`:
