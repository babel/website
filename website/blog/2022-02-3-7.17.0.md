...
After years of [iterations](https://babeljs.io/blog/2018/09/17/decorators), rewrites and adjustments it looks like the [decorators proposal](github.com/tc39/proposal-decorators) has finally stabilized again on a new design! This release includes parse and transform support for the new proposal.

We also implemented the [RegExp `v` flag proposal](https://github.com/tc39/proposal-regexp-set-notation), added parsing support for [destructuring private fields](https://github.com/tc39/proposal-destructuring-private), and a new experimental implementation of `@babel/register`.
...
After receiving feedback from JavaScript developers, engine developers and language experts, [the decorators proposal](https://github.com/tc39/proposal-decorators/) was rewritten taking into account multiple constraints and desires. We collaborated with the current proposal author, [@pzuraq](https://www.pzuraq.com/), and they implemented the new stage 2 proposal in Babel.
...
class Button {
  @reactive accessor enabled = true;
...
> ⚠️ The new decorators syntax is compatible with the previous proposal, but the semantics are different: decorators written for Babel _"legacy"_, Babel _older-but-not-legacy_ or TypeScript will not work with the new proposal.

If you want to help moving the proposal forward, you can try using it and leave feedback in [their GitHub repository](https://github.com/tc39/proposal-decorators/issues).
...
We are continuing to help the proposal authors evaluating different tokens for the [pipeline operator](https://github.com/tc39/proposal-pipeline-operator) proposal: one of the champions, [J. S. Choi](https://jschoi.org/), added support for two new tokens that you can try with the `topicToken` option:
