...
### Nullish coalescing operator (`??`) updates

The nullish coaleshing stage 3 proposal recently got some updates: to avoid confusion over precedence with other logical operators (`&&` and `||`), it has been decided to disallow mixing them.
...
Instead, you should properly add parentheses where appropriate:
```javascript=
(a ?? b) || c; /* or */ a ?? (b || c);
(a && b) ?? c; /* or */ a && (b ?? c);
```
