...
For these reasons, they can only be used when calling Babel via the promise-based or callback-based entry points. They already work with `@babel/cli`, `babel-loader` and `gulp-babel`, and they will work with the next release of `rollup-plugin-babel`. They are not supported by `babel-eslint` yet.
...
While we don't anticipate a huge migration path, here are two situations which we want to bring to your attention:
