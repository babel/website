...
* `babel-eslint` has historically been maintained by the Babel team, and ensuring that ESLint's core rules (which is released every two weeks) work with experimental syntax is a monumental task.
* Babel's internal AST representation is different from [ESTree](https://github.com/estree/estree), the AST specification used by ESLint. As a result, Babel's parser has a plugin that outputs ASTs that are ESTree-compatible.
...
* The version of Babel's parser that `babel-eslint` relies on is a direct dependency in `package.json` and the plugins enabled are hardcoded in project's source code, leading to a potential mismatch in versions and enabled language features between compiling and linting. This has historically led to a lot of confusion and hard-to-debug issues, and we believe this is a big win for both maintainers and end users.
...
The challenges above have added up to `babel-eslint` requiring more resources than the team has, resulting in `babel-eslint` not getting the attention a widely used project (6M downloads a week at the time of this writing) requires to stay up-to-date.
