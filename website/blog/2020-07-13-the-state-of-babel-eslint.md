...
* The version of Babel's parser that `babel-eslint` relies on is a direct dependency in `package.json` and the plugins enabled are hardcoded in project's source code, leading to a potential mismatch in versions and enabled language features between compiling and linting. This has historically led to a lot of confusion and hard-to-debug issues for both maintainers and end users.
