...
`core-js@3` was just released, and includes [a lot of new features](https://github.com/zloirock/core-js/blob/master/CHANGELOG.md#300). Other than all those new proposals, it makes it possible to polyfill transform _instance_ methods using `@babel/plugin-transform-runtime` so that they can be used in old browsers without polluting the global environment:
...
TypeScript 3.4 RC was released a few days ago, and thanks to [Tan Li Hau](https://github.com/tanhauhau) it is already supported by Babel!
