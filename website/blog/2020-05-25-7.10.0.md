...
In TypeScript 3.8 and Babel 7.9, the above would be read as `(foo?.bar)!.baz`: "If `foo` is not nullish, get `.bar` from it. Then trust me that `foo?.bar` is never nullish and always get `.bar` from it". This means that when `foo` is nullish that code would always throw, because we are trying to get `.baz` from `undefined`.
...
Thanks to [Devon](https://twitter.com/devongovett) from the Parcel team, `@babel/preset-react` now injects `/*#__PURE__*/` annotations in those functions calls to mark them as being safe to be tree-shaken away.
