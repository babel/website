...
These packages all support a `method` option for adjusting how they're injected (essentially what `@babel/preset-env` and `@babel/plugin-transform-runtime` offer). You can inject a polyfill into an entry point (global scope only) or by usage in your code (both global scope and "pure" options).
