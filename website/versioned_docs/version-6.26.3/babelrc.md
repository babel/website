...
All Babel API [options](core#options) except the callbacks are allowed (because `.babelrc` files are serialized as [JSON5](https://github.com/json5/json5)).
...
Use `"babelrc": false` in [options](core#options) to stop lookup behavior, or provide the [`--no-babelrc` CLI flag](babel-cli#babel-ignoring-babelrc).
